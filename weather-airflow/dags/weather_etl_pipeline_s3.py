...
def transform_load_data(ti):
    ...
    # Upload to S3
    df.to_csv(f"s3://{bucket}/{filename}", index=False,
...
with DAG(
    dag_id="openweathermap_etl_pipeline",
    description="Daily ETL pipeline for OpenWeatherMap data to AWS S3",
    default_args=default_args,
    start_date=datetime(2026, 7, 1),
    schedule="*/5 * * * *",
    catchup=False,
    tags=["weather", "etl", "aws", "s3"],
) as dag:
